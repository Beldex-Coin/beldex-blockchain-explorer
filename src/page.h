...
time_t calculate_service_node_expiry_timestamp(uint64_t registration_height)
{
    ...
    expiry_height += master_nodes::staking_num_lock_blocks(nettype);
...
void generate_service_node_mapping(mstch::array *array, bool on_homepage, std::vector<COMMAND_RPC_GET_MASTER_NODES::response::entry *> const *entries)
{
    ...
    {
        COMMAND_RPC_GET_MASTER_NODES::response::entry const *entry = (*entries)[i];
        ...
        {
          {"public_key",                    entry->master_node_pubkey},
...
render_service_nodes_html(bool add_header_and_footer)
{
    ...
    {
        registered.reserve  (response.master_node_states.size());
        unregistered.reserve(response.master_node_states.size() * 0.5f);

        for (auto &entry : response.master_node_states)
        {
...
render_quorum_states_html(bool add_header_and_footer)
{
    ...
    if (on_homepage)
    {
      if (block_height >= master_nodes::quorum_cop::REORG_SAFETY_BUFFER_IN_BLOCKS)
        block_height -= master_nodes::quorum_cop::REORG_SAFETY_BUFFER_IN_BLOCKS;
      else
        block_height = 0;
    }
    ...
    for (const auto& entry : sn_response.master_node_states)
    {
        pk2sninfo.insert({entry.master_node_pubkey, entry});
    }
...
show_service_node(const std::string &service_node_pubkey)
{
    ...
    mstch::map page_context {};
    COMMAND_RPC_GET_MASTER_NODES::response::entry const *entry = &response.master_node_states[0];

    // Make metadata render data
    static std::string friendly_uptime_proof_not_received = "Not Received";
    int operator_cut_in_percent = portions_to_percent(entry->portions_for_operator);

    page_context["public_key"]           = entry->master_node_pubkey;
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    if (tx.version == transaction::version_3_per_output_unlock_times)
    {
        ...
        tx_extra_master_node_register   register_;
        if (tx.get_type() == cryptonote::transaction::type_deregister)
        {
            ...
            if (get_master_node_deregister_from_tx_extra(tx.extra, deregister))
            {
              ...
              for (tx_extra_master_node_deregister::vote &vote : deregister.votes)
              {
                mstch::map entry
                {
        ...
        else if (get_master_node_register_from_tx_extra(tx.extra, register_))
        {
            ...
            context["register_signature"]             = pod_to_hex(register_.m_master_node_signature);
